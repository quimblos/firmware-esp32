...
static void blufi_dh_pregen_start_impl(void (*done_cb)(void))
{
    ...
    if (s_pregen_done == NULL) {
        ESP_LOGE(BluFi::TAG, "Failed to create pre-gen semaphore");
        return;
    }
    ...
    if (xTaskCreate(blufi_dh_pregen_task, "blufi_pregen",
        ...
        ESP_LOGE(BluFi::TAG, "Failed to create pre-gen task");
        vSemaphoreDelete(s_pregen_done);
        s_pregen_done = NULL;
    }
}

void BluFi::blufi_dh_pregen_start(void)
{
    blufi_dh_pregen_start_impl(NULL);
}

void BluFi::blufi_dh_pregen_start_with_cb(void (*done_cb)(void))
{
    blufi_dh_pregen_start_impl(done_cb);
}

void BluFi::blufi_dh_pregen_wait(void)
{
    if (s_pregen_done == NULL) {
        return;
    }
...
static void dh_pregen_cleanup(void)
{
    if (s_pregen_done != NULL) {
        xSemaphoreTake(s_pregen_done, portMAX_DELAY);
        vSemaphoreDelete(s_pregen_done);
...
void BluFi::dh_negotiate_data_handler(uint8_t *data, int len, uint8_t **output_data, int *output_len, bool *need_free)
{
    if (data == NULL || len < 3) {
        ESP_LOGE(BluFi::TAG, "BLUFI Invalid data format");
    ...
    if (output_data == NULL || output_len == NULL || need_free == NULL) {
        ESP_LOGE(BluFi::TAG, "BLUFI Invalid output parameters");
        btc_blufi_report_error(ESP_BLUFI_DATA_FORMAT_ERROR);
        return;
    }
    ...
    if (blufi_sec == NULL) {
        ESP_LOGE(BluFi::TAG, "BLUFI Security is not initialized");
    ...
    case SEC_TYPE_DH_PARAM_LEN:
        ...
        if (blufi_sec->dh_param_len == 0 || blufi_sec->dh_param_len > DH_PARAM_LEN_MAX) {
            ESP_LOGE(BluFi::TAG, "%s, invalid dh param len %d\n", __func__, blufi_sec->dh_param_len);
        ...
        if (blufi_sec->dh_param == NULL) {
            ...
            ESP_LOGE(BluFi::TAG, "%s, malloc failed\n", __func__);
            return;
        }
        break;
    case SEC_TYPE_DH_PARAM_DATA:{
        if (blufi_sec->dh_param == NULL) {
            ESP_LOGE(BluFi::TAG, "%s, blufi_sec->dh_param == NULL\n", __func__);
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }

        if (len < (blufi_sec->dh_param_len + 1)) {
            ESP_LOGE(BluFi::TAG, "%s, invalid dh param len\n", __func__);
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (blufi_sec->dh_param_len < 2) {
            ESP_LOGE(BluFi::TAG, "DH param too short for P length");
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (p_offset > (size_t)blufi_sec->dh_param_len) {
            ESP_LOGE(BluFi::TAG, "P length %d exceeds dh_param bounds", p_len);
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        param += p_offset;
        ...
        if ((param - blufi_sec->dh_param) + 2 > (size_t)blufi_sec->dh_param_len) {
            ESP_LOGE(BluFi::TAG, "DH param too short for G length");
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (g_offset > (size_t)blufi_sec->dh_param_len) {
            ESP_LOGE(BluFi::TAG, "G length %d exceeds dh_param bounds", g_len);
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        param += 2 + g_len;
        ...
        if ((param - blufi_sec->dh_param) + 2 > (size_t)blufi_sec->dh_param_len) {
            ESP_LOGE(BluFi::TAG, "DH param too short for public key length");
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (pub_offset > (size_t)blufi_sec->dh_param_len) {
            ESP_LOGE(BluFi::TAG, "Public key length %d exceeds dh_param bounds", pub_len);
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (key_bits != 3072) {
            ESP_LOGE(BluFi::TAG, "Unsupported DH key length: %d bytes (only 3072 bit keys supported)", pub_len);
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (pub_len > DH_SELF_PUB_KEY_LEN) {
            ESP_LOGE(BluFi::TAG, "Public key length %d exceeds output buffer size %d", pub_len, DH_SELF_PUB_KEY_LEN);
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (!used_pregen) {
            ...
            if (status != PSA_SUCCESS) {
                ESP_LOGE(BluFi::TAG, "%s psa_generate_key failed %d\n", __func__, status);
                blufi_wdt_set_timeout(CONFIG_ESP_TASK_WDT_TIMEOUT_S * 1000);
                btc_blufi_report_error(ESP_BLUFI_DH_MALLOC_ERROR);
                blufi_cleanup_dh_param();
                return;
            ...
            if (status != PSA_SUCCESS) {
                ESP_LOGE(BluFi::TAG, "%s psa_export_public_key failed %d\n", __func__, status);
                blufi_wdt_set_timeout(CONFIG_ESP_TASK_WDT_TIMEOUT_S * 1000);
        ...
        if (status != PSA_SUCCESS) {
            ESP_LOGE(BluFi::TAG, "%s psa_raw_key_agreement failed %d\n", __func__, status);
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (blufi_sec->share_len > SHARE_KEY_LEN) {
            ESP_LOGE(BluFi::TAG, "Share key length %d exceeds buffer size %d", blufi_sec->share_len, SHARE_KEY_LEN);
            blufi_cleanup_dh_param();
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (status != PSA_SUCCESS) {
            ESP_LOGE(BluFi::TAG, "%s psa_hash_compute failed %d\n", __func__, status);
        ...
        if (status != PSA_SUCCESS) {
            ESP_LOGE(BluFi::TAG, "%s psa_import_key failed %d\n", __func__, status);
            btc_blufi_report_error(ESP_BLUFI_DH_MALLOC_ERROR);
            blufi_cleanup_dh_param();
            return;
        }
        ...
        if (status != PSA_SUCCESS) {
            ESP_LOGE(BluFi::TAG, "%s encryption IV derivation failed %d\n", __func__, status);
            btc_blufi_report_error(ESP_BLUFI_DH_MALLOC_ERROR);
        ...
        if (status != PSA_SUCCESS) {
            ESP_LOGE(BluFi::TAG, "%s decryption IV derivation failed %d\n", __func__, status);
            btc_blufi_report_error(ESP_BLUFI_DH_MALLOC_ERROR);
            blufi_cleanup_negotiation(false, false);
            return;
        }
        ...
        if (status != PSA_SUCCESS) {
            ESP_LOGE(BluFi::TAG, "%s encryption setup failed %d\n", __func__, status);
            btc_blufi_report_error(ESP_BLUFI_DH_MALLOC_ERROR);
            blufi_cleanup_negotiation(false, false);
            return;
        }
        ...
        if (status != PSA_SUCCESS) {
            ESP_LOGE(BluFi::TAG, "%s encryption IV setup failed %d\n", __func__, status);
            btc_blufi_report_error(ESP_BLUFI_DH_MALLOC_ERROR);
        ...
        if (status != PSA_SUCCESS) {
            ESP_LOGE(BluFi::TAG, "%s decryption setup failed %d\n", __func__, status);
            btc_blufi_report_error(ESP_BLUFI_DH_MALLOC_ERROR);
            blufi_cleanup_negotiation(true, false);
            return;
        }
        ...
        if (status != PSA_SUCCESS) {
            ESP_LOGE(BluFi::TAG, "%s decryption IV setup failed %d\n", __func__, status);
            btc_blufi_report_error(ESP_BLUFI_DH_MALLOC_ERROR);
    ...
    default:
        ...
        ESP_LOGE(BluFi::TAG, "Unknown packet type: 0x%02x", type);
        btc_blufi_report_error(ESP_BLUFI_DATA_FORMAT_ERROR);
        break;
    }
}

int BluFi::aes_encrypt(uint8_t iv8, uint8_t *crypt_data, int crypt_len)
{
    ...
    if (blufi_sec->aes_key == 0) {
        ESP_LOGE(BluFi::TAG, "AES key is not initialized");
        return -1;
    }
    ...
    if (output_buf == NULL) {
        ESP_LOGE(BluFi::TAG, "Failed to allocate output buffer");
        return -1;
    }
    ...
    if (status != PSA_SUCCESS) {
        ESP_LOGE(BluFi::TAG, "Encryption failed with status=%d", status);
        free(output_buf);
        return -1;
    }

    if (output_len > (size_t)crypt_len) {
        ESP_LOGE(BluFi::TAG, "Output length %d exceeds input buffer size %d", output_len, crypt_len);
        free(output_buf);
        return -1;
    }
...
int BluFi::aes_decrypt(uint8_t iv8, uint8_t *crypt_data, int crypt_len)
{
    (void)iv8;  /* iv8 parameter is no longer used with persistent CTR operation */
    ...
    psa_status_t status = psa_cipher_update(&blufi_sec->dec_operation, crypt_data, crypt_len,
                                           output_buf, output_buf_size, &output_len);
    if (status != PSA_SUCCESS) {
        ESP_LOGE(BluFi::TAG, "Decryption failed with status=%d", status);
        free(output_buf);
        return -1;
    }
...
uint16_t BluFi::crc_checksum(uint8_t iv8, uint8_t *data, int len)
{
    /* This iv8 ignore, not used */
    return esp_crc16_be(0, data, len);
}

esp_err_t BluFi::init_security(void)
{
...
void BluFi::deinit_security(void)
{
    if (blufi_sec == NULL) {
        return;
    }
