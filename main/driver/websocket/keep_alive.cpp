...
{
    client_fd_action_t stop = { .type = STOP_TASK, .fd = -1, .last_seen = 0 };
...
{
    client_fd_action_t client_fd_action = { .type = CLIENT_FD_ADD, .fd = fd, .last_seen = 0 };
...
{
    client_fd_action_t client_fd_action = { .type = CLIENT_FD_REMOVE, .fd = fd, .last_seen = 0 };
    if (xQueueSendToBack(h->q, &client_fd_action, 0) == pdTRUE) {
        return ESP_OK;
    }
    return ESP_FAIL;
