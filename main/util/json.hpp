...
struct JSON {
    ...
    bool has(const std::string& key) const {
        if (children.contains(key)) return children.at(key).kind != EMPTY;
        return false;
    }
    ...
    private:
        // NOTICE: The section below was generated by DeepSeek Flash 4.1
        // and requires manual review for coding standards fit.
...
template <>
inline uint8_t JSON::as() const {
    auto str = value.c_str();
    char* end = nullptr;
    const long long v = std::strtoll(str, &end, 10);
    return end == str ? 0 : v;
}

template <>
inline uint16_t JSON::as() const {
    auto str = value.c_str();
    char* end = nullptr;
    const long long v = std::strtoll(str, &end, 10);
    return end == str ? 0 : v;
